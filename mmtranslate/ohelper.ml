...
(* Each token must start with \: Windows native path without drive name *)
let process_entry str =
  (* non-zero length because of Str.split *)
  if str.[0] <> '\\' then begin
    (* CygwinPath.to_native str *)
    Printf.eprintf "mmtranslate: this mmtranslate cannot handle this path: %s\n%!" str;
    str
  end else
    str
...
let rec cat buf =
  let further =
    try
      (* split by spaces and send them to handle_entry *)
